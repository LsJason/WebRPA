...
- 打开网页、关闭页面、刷新页面、前进、后退
- 点击元素（单击/双击/右键）、悬停元素
- 输入文本、下拉框选择、复选框操作
- 拖拽元素、滚动页面、真实鼠标滚动
- 处理弹窗对话框（确认/取消/输入）
- 可视化元素选择器（支持单选/批量选择相似元素）
...
- 列表操作（追加/插入/删除/弹出/清空/反转/排序/去重/合并）
- 列表取值、列表长度
- 字典操作（设置/删除键值）
- 字典取值、获取所有键
...
- API 请求（GET/POST/PUT/DELETE/PATCH）
- 发送邮件（SMTP）

#### 🤖 AI 能力（2个模块）

- AI 对话（支持 OpenAI、智谱、Deepseek、月之暗面等）
- AI 视觉（图像识别与理解）
...
- 打印日志（info/success/warning/error）
- 播放提示音、播放音乐
- 语音播报（TTS）
- 用户输入弹窗
